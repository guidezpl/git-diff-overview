...
module.exports = class GitDiffView {
  ...
  start () {
    ...
    this.subscriptions.add(
      ...
      atom.config.onDidChange('git-diff-plus.showMarkersInScrollBar', () =>
      this.updateIconDecoration()),
  ...
  updateIconDecoration () {
    ...
    if (scrollMarkerView) {
      if (atom.config.get('git-diff-plus.showMarkersInScrollBar')) {
  ...
  addDecorations (diffs) {
    for (const { newStart, oldLines, newLines } of diffs) {
      ...
      console.log(diffs, startRow, endRow)
      if (oldLines === 0 && newLines > 0) {
        this.markRange(startRow, endRow, 'git-line-added')
        this.scrollAddLayer.addMarkers(startRow, endRow)
      } else if (newLines === 0 && oldLines > 0) {
        if (startRow < 0) {
          this.markRange(0, 0, 'git-previous-line-removed')
          this.scrollRemoveLayer.addMarker(0)
        } else {
          this.markRange(startRow, startRow, 'git-line-removed')
          this.scrollRemoveLayer.addMarker(startRow)
        }
      } else {
        this.markRange(startRow, endRow, 'git-line-modified')
        this.scrollEditLayer.addMarkers(startRow+1, endRow)
      }
    }
  }
